...
def display(txt,size,save_name):
	...
	while i <= len(points)-3:
		if points[i+2][0]==0 and points[i+2][1]==0:
			Bezier(img,points[i],points[i+1],start_point)
			#print '[0,0]'
			start_point = points[i+3]
			i = i + 3
		elif points[i+1][0]==0 and points[i+1][1]==0:
			Bezier(img,points[i],start_point,start_point)
			#print '[0,0]'
			start_point = points[i+2]
			i = i + 2
		else:
			Bezier(img,points[i],points[i+1],points[i+2])
			i = i + 2

	Bezier(img,points[len(points)-2],points[len(points)-1],start_point)
